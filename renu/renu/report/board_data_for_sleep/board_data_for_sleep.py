...
def get_columns(filters):
	...
	for o in  operation:
		...
		sub_operation = frappe.get_all("Sub Operation",fields = ['operation',],filters = {'parent':o.operation}, distinct="operation")
		if sub_operation:
			for s in sub_operation:
				each_sub_col=	{
							"fieldname": "--"+str(s.operation),
							"fieldtype": "Data",
							"label": "--"+str(s.operation),

						}
				coloumns.append(each_sub_col)
	return coloumns
...
def get_data(filters):
	...
	for i in pcb_details:
		...
		operation_data = 	frappe.db.sql("""
											SELECT a.operation, b.product_type , a.name
											FROM `tabJob Card` a
		...
		# frappe.throw(str(operation_data))
		for j in operation_data:
			each_parent_dict = {str(j.operation) : str(j.product_type)}
			row_dict = {**row_dict ,**each_parent_dict}


			sub_operation = frappe.get_all("Sub Operation",fields = ['operation',],filters = {'parent':j.operation}, distinct="operation")
			
			for k in sub_operation:
				for_each_operation = frappe.get_all("Sub Process Details",fields = ['result_description',],filters = {'qr_id':i.qr_code , 'sub_operation' : k.operation})
				# frappe.throw(str(for_each_operation))
				for d in for_each_operation:
					each_sub_dict = {"--"+str(k.operation) : str(d.result_description)}
					row_dict = {**row_dict ,**each_sub_dict}
